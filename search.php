...
//var_dump("Initial = " . $keyword . "<br>");

if($keyword === '') {
    $keyword = $_GET['title'];
    //var_dump("Condition = ". $keyword . "<br>");
}
//var_dump("Final = ". $keyword . "<br>");

$products->generateSearchResult($keyword);


$titles = $products->getSearchResultTitles();
...
<div id="rightnav">
	...
	<div class="product-list">
		...
		<?php 
			...
			$ttl = isset($_GET['title']) ? $_GET['title'] : '';
			$nextpageInfo = "search.php?keyword=" . $keyword . "&title=". $ttl ."&search=Search+Products&page=search&page=";
		
		?>
		<a class="pages" href=<?php echo $nextpageInfo . $prev; ?>>&lt;prev</a>
		&nbsp;|&nbsp;
		<a class="pages" href=<?php echo $nextpageInfo . $next; ?>>next&gt;</a>
		<?php echo ($message) ? "&nbsp;&nbsp;<b>$message<br />" : ''; ?>
		<ul>
			<?php echo $view->displayProducts($page, $linesPerPage, $maxProducts, $products->getSearchResult()); ?>		
		</ul>
	</div><!-- product-list -->
